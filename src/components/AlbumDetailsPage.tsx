import { ArrowLeft, ExternalLink, CreditCard, Package, Sparkles, Star, Award, TrendingUp, ShoppingCart, Zap } from 'lucide-react';
  ...
  const getStoresByContinent = (continent: ContinentType) => {
    const stores = {
      ...
      korea: {
        ...
        physical: [
          { name: 'Weverse Shop KR', url: 'https://weverse.io/blackpink/feed', description: 'Official albums - Counts for Circle Chart', icon: '🌐' },
          { name: 'YG Select', url: 'https://en.ygselect.com/', description: 'Official YG albums & merch', icon: '🏪' },
          { name: 'Synnara', url: 'https://www.synnara.co.kr/', description: 'Major retailer - Circle certified', icon: '🏬' },
          { name: 'Yes24', url: 'http://www.yes24.com/searchcorner/Search?keywordAd=&keyword=&domain=ALL&qdomain=%EC%A0%84%EC%B2%B4&Wcode=001_005&query=BLACKPINK', description: 'Large selection - Chart counting', icon: '✅' },
          { name: 'Aladin', url: 'https://www.aladin.co.kr/search/wsearchresult.aspx?SearchTarget=All&SearchWord=BLACKPINK', description: 'Popular store - Fast shipping', icon: '📚' },
        ],
      ...
      usa: {
        ...
        physical: [
          { name: 'BLACKPINK US Shop', url: 'https://shop.blackpinkmusic.com/', description: 'Official store - Billboard counting', icon: '🖤' },
          { name: 'Target', url: 'https://www.target.com/s?searchTerm=blackpink+album', description: 'Exclusive editions - Major retailer', icon: '🎯' },
          { name: 'Walmart', url: 'https://www.walmart.com/search?query=blackpink%20album', description: 'Nationwide availability', icon: '🏪' },
          { name: 'Amazon US', url: 'https://www.amazon.com/s?k=blackpink+album', description: 'Fast shipping - Chart eligible', icon: '📦' },
          { name: 'Barnes & Noble', url: 'https://www.barnesandnoble.com/s/blackpink', description: 'Premium packaging options', icon: '📚' },
        ],
        ...
        <div className="relative z-20 p-8 overflow-y-auto">
          <div className="max-w-7xl mx-auto">
            ...
            <div className="grid lg:grid-cols-2 gap-8 mb-12">
              ...
              {/* Physical */}
              <div className="group relative bg-gradient-to-br from-purple-900/40 to-blue-900/40 backdrop-blur-xl border-2 border-purple-500/40 rounded-3xl p-8 hover:border-purple-400 transition-all duration-500 shadow-2xl hover:shadow-purple-500/40">
                ...
                <div className="relative z-10">
                  ...
                  <div className="space-y-4">
                    {storeData.physical.map((store, i) => (
                      <a key={i} href={store.url} target="_blank" rel="noopener noreferrer"
                        className="block bg-black/40 hover:bg-black/60 rounded-2xl p-6 transition-all duration-300 group/item border border-purple-500/20 hover:border-purple-400/60 transform hover:-translate-y-1">
                        <div className="flex items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-3 mb-2">
                              <span className="text-2xl">{store.icon}</span>
                              <h3 className="text-xl font-bold text-white group-hover/item:text-purple-300 transition-colors">{store.name}</h3>
                            </div>
                            <p className="text-gray-300 text-sm leading-relaxed">{store.description}</p>
                          </div>
                          <ExternalLink className="w-5 h-5 text-gray-400 group-hover/item:text-purple-400 transition-all duration-300 flex-shrink-0 group-hover/item:scale-125" />
                        </div>
                      </a>
                    ))}
                  </div>
                  <div className="mt-8 p-6 bg-gradient-to-br from-purple-900/40 to-blue-900/40 rounded-2xl border border-purple-500/30">
                    <div className="flex items-center gap-3 mb-4">
                      <ShoppingCart className="w-6 h-6 text-purple-400" />
                      <h3 className="text-lg font-bold text-purple-300">Physical Buying Tips</h3>
                    </div>
                    <ul className="space-y-3 text-gray-300 text-sm">
                      {['Buy from certified retailers for chart counting', 'Look for exclusive variants & inclusions', 'First-week sales are crucial', 'Includes photocards & collectibles', 'Boosts physical chart positions'].map((tip, i) => (
                        <li key={i} className="flex items-start gap-3">
